...
def validate_file(file_path: str) -> bool:
    ...
    if not path.exists():
        ...
        return False
    if path.suffix.lower() not in ('.xlsx', '.xls', '.csv', '.xlsm', '.xlsb', '.ods', '.txt'):
...
def load_excel(file_path: str,
    ...
    try:
        if engine:
            df = pd.read_excel(path_obj, engine=engine, skiprows=skiprows, nrows=max_rows)
        else:
...
def apply_transformation(df: pd.DataFrame,
                         config_columns: Dict[str, Any],
                         mode: str) -> Tuple[pd.DataFrame, Optional[float]]:
    """
    ...
    if mode == "fedex":
        ...
        grouped = df_f.groupby('masterTrackingNumber').agg({
            'shipDate': 'first',
            'reference': 'first',
            'recipientCity': 'first',
