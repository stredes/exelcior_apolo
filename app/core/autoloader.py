...
def is_fedex_pattern(filename: str) -> bool:
    return (
        re.match(r"^shipment_report_\d{4}-\d{2}-\d{2}$", Path(filename).stem.lower())
        is not None
    )
...
def find_latest_file_by_mode(
    ...
    if not download_folder.exists():
        capturar_log_bod1(
            f"📂 Carpeta de descargas no encontrada: {download_folder}", "warning"
        )
    ...
    if not archivos_filtrados:
        capturar_log_bod1(
            f"❌ No hay archivos válidos para el modo '{mode}' en {download_folder}",
            "warning",
        )
        return None, "no_match"

    archivos_ordenados = sorted(
        archivos_filtrados, key=lambda f: f.stat().st_mtime, reverse=True
    )
