...
class ConfigDialog(tk.Toplevel):
    def __init__(
        ...
        config_columns: Dict[str, Dict[str, Set[str]]],
    ) -> None:
        ...
        self.start_row: tk.IntVar = tk.IntVar(
            value=self.config_columns.get(mode, {}).get("start_row", 0)
        )
        self.nombre_archivo_digitos: tk.StringVar = tk.StringVar(
            value=",".join(
                map(
                    str,
                    self.config_columns.get(mode, {}).get("nombre_archivo_digitos", []),
                )
            )
        )
        self.preview_font_size: tk.IntVar = tk.IntVar(
            value=self.config_columns.get(mode, {}).get("vista_previa_fuente", 10)
        )
    ...
    def _create_widgets(self) -> None:
        ...
        frame_sumar = ttk.LabelFrame(
            container, text="Columnas para sumatorias", padding=10
        )
        frame_sumar.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
        lbl_info = tk.Label(
            frame_sumar,
            text="Las celdas se convertirán a numérico para sumar",
            bg="#ffffff",
        ...
        frame_preservar = ttk.LabelFrame(
            container, text="Columnas a preservar (sin modificar formato)", padding=10
        )
        frame_preservar.grid(
        ...
        frame_row = ttk.LabelFrame(
            container, text="Fila de inicio de datos (start_row)", padding=10
        )
        frame_row.grid(row=2, column=0, sticky="ew", padx=5, pady=5)
        self.spinbox_row = ttk.Spinbox(
            frame_row, from_=0, to=20, textvariable=self.start_row, width=5
        )
        self.spinbox_row.pack(pady=5)

        if self.mode == "urbano":
            frame_digitos = ttk.LabelFrame(
                container, text="Dígitos del nombre del archivo (urbano)", padding=10
            )
            frame_digitos.grid(row=2, column=1, sticky="ew", padx=5, pady=5)
            self.entry_digitos = ttk.Entry(
                frame_digitos, textvariable=self.nombre_archivo_digitos, width=15
            )
            self.entry_digitos.pack(pady=5)

        if self.mode in ("fedex", "urbano", "listados"):
            frame_fuente = ttk.LabelFrame(
                container, text="Tamaño de fuente de vista previa", padding=10
            )
            frame_fuente.grid(
                row=3, column=0, columnspan=2, sticky="ew", padx=5, pady=5
            )
            self.spin_font = ttk.Spinbox(
                frame_fuente,
                from_=8,
                to=24,
                textvariable=self.preview_font_size,
                width=5,
            )
        ...
        ttk.Button(
            btn_frame, text="Restablecer por defecto", command=self._reset_to_default
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Guardar", command=self._on_save).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _on_save(self) -> None:
        self.selected_eliminar = {
            self.available_columns[i] for i in self.listbox_eliminar.curselection()
        }
        self.selected_sumar = (
            {self.available_columns[i] for i in self.listbox_sumar.curselection()}
            if self.listbox_sumar
            else set()
        )
        self.selected_preservar = {
            self.available_columns[i] for i in self.listbox_preservar.curselection()
        }
        ...
        try:
            nombre_digitos = (
                [
                    int(x.strip())
                    for x in self.nombre_archivo_digitos.get().split(",")
                    if x.strip().isdigit()
                ]
                if self.mode == "urbano"
                else []
    ...
    def _reset_to_default(self):
        if self.mode in DEFAULT_CONFIG:
            ...
            if self.mode == "urbano":
                self.nombre_archivo_digitos.set(
                    ",".join(
                        map(
                            str,
                            DEFAULT_CONFIG[self.mode].get("nombre_archivo_digitos", []),
                        )
                    )
                )
            self.preview_font_size.set(
                DEFAULT_CONFIG[self.mode].get("vista_previa_fuente", 10)
            )
            self._load_initial_selection()

            log_evento(
                f"Configuración restablecida por defecto para modo: {self.mode}",
                "warning",
            )
