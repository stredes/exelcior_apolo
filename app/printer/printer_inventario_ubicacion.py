...
def _format_inventory_sheet(sheet, df: pd.DataFrame, total_cantidad: int, titulo: str) -> None:
    ...
    for row in sheet.iter_rows(min_row=3, max_row=data_end_row, min_col=1, max_col=total_columnas):
        for cell in row:
            cell.font = body_font
...
def _cleanup_temp_file_later(file_path: Path, delay_seconds: int = 180):
    def _cleanup():
        ...
        try:
            ...
            log_evento(
                f"No se pudo eliminar temporal de inventario por ubicacion: {cleanup_error}",
                "warning",
            )
